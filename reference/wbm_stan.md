...
A near-equivalent of [`wbm()`](wbm.md) that instead uses Stan, via rstan
and brms.
...
- model.cor:
  ...
  to be consistent with [`wbm()`](wbm.md), reduce runtime, and avoid
  warnings for non-linear models.
...
See [`wbm()`](wbm.md) for details on the formula syntax, model types,
