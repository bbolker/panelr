...
[`panel_data()`](panel_data.md) data frames. It's a workhorse in
[`wbm()`](wbm.md) but may be useful in scripting use as well.
...
- data:
  ...
  A [`panel_data()`](panel_data.md) frame.

## Value

A [`panel_data()`](panel_data.md) frame with only the columns needed to
fit a model as described by the formula.
