...
As an example, we can look at the [WageData](WageData.md) included in
this package. We will create a model that predicts the logarithm of the
individual's wages (`lwage`) with their union status (`union`), which
can change over time, and their race (`blk`; dichotomized as black or
non-black), which does not change throughout the period of study. Our
...
[`wbm_stan()`](wbm_stan.md) for a Bayesian estimation option.
