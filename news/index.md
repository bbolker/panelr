...
- Models should now be compatible with `sim_slopes()` and
  `johnson_neyman()` from the `interactions` package. Note that this
  compatibility is pending an update to that package.
...
- Includes back-end changes for compatibility with recent updates to
  ...
  behavior from these.
- When users do not want [`long_panel()`](../reference/long_panel.md) to
  check whether values are varying, it no longer performs a series of
...
- Very small performance improvement when using
  [`long_panel()`](../reference/long_panel.md) and
  [`are_varying()`](../reference/are_varying.md) functions.
...
- [`long_panel()`](../reference/long_panel.md) now handles numeric waves
  correctly when the input data are unbalanced.
...
- Added `interaction.style` argument to
  [`make_wb_data()`](../reference/make_wb_data.md).
- The behavior [`predict.wbm()`](../reference/predict.wbm.md) and
  [`predict.wbgee()`](../reference/predict.wbgee.md) has been improved.
  Notably, the DV does not need to be included in `newdata` and the ID
  variable is only required when necessary.
...
- [`wbgee()`](../reference/wbgee.md) works just like
  [`wbm()`](../reference/wbm.md), except uses GEE (via the `geepack`
  package) for estimation. This can give you more trustworthy results
  under some circumstances and is much less likely to have convergence
  problems.
- [`fdm()`](../reference/fdm.md) estimates first differences models via
  GLS (from the `nlme` package).
- [`asym()`](../reference/asym.md) estimates the linear asymmetric
  effects model described by [Allison
  (2019)](https://journals.sagepub.com/doi/10.1177/2378023119826441) via
  first differences.
- [`asym_gee()`](../reference/asym_gee.md) estimates a similar
  asymmetric effects model to the one using cumulative differences
  described in Allison (2019), but using GEE rather than conditional
  logit.
- [`heise()`](../reference/heise.md) produces stability and reliability
  estimates via the popular method described in Heise (1969).
...
[`are_varying()`](../reference/are_varying.md) can now also assess
individual-level variation, so using the `type = "individual"` argument
you can instead assess variables like age that vary over time but change
equally for every case. \* [`wbm()`](../reference/wbm.md) can now handle
...
[`get_periods()`](../reference/get_wave.md) function. For example, if
the waves in your data are the numbers 1 through 7, that’s what you’ll
get. This is more useful when the periods are irregular, such as if the
waves are the years of a biennial survey.
...
[`wbm()`](../reference/wbm.md) in the formula without running into
cryptic errors. \* Fixed a problem in which transformed variables (like
`lag(x)`) could not be included as a user-specified random effect.
Pre-0.5.0, these could be included if they were surrounded by backticks,
but now that hack is unnecessary and does not work. \*
[`make_wb_data()`](../reference/make_wb_data.md) is now updated to work
with other internal updates introduced in 0.5.0. \*
[`long_panel()`](../reference/long_panel.md) was never really working
right when the source data’s labels were located at the beginning (i.e.,
`label_location = "beginning"`). It is now much more robust. \*
[`wbm()`](../reference/wbm.md)’s `wave.factor` argument had become
non-functional for some time but is now fixed.
...
- `panel_data` frames now always place the `id` and `wave` columns first
  (in that order).
- [`wbm()`](../reference/wbm.md) can now handle time-varying factors
  appropriately. Do note that it only uses treatment contrasts, however.
  ([\#8](https://github.com/jacob-long/panelr/issues/8))
- There is a new function, [`line_plot()`](../reference/line_plot.md),
...
- [`as_panel_data()`](../reference/panel_data.md) is an alias for
  [`panel_data()`](../reference/panel_data.md) when supplying a data
  frame and an S3 method otherwise. It can be used to convert
  `pdata.frame` objects from the `plm` package to `panel_data`.
- Formulas provided to [`wbm()`](../reference/wbm.md) are now converted
  to `Formula` objects to make working with their multiple parts easier
  (see the `Formula` package for more info).
